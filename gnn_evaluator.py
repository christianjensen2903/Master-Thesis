...
class GNNEvaluator:
    """
    Evaluator for dual encoder GNN models.

    Key difference from standard evaluator:
    - Query embeddings are computed using encode_query() (MLP only, no graph)
    - Document embeddings are computed using encode_documents() (GNN with graph)
    """
    ...
    def __init__(
        ...
        # Compute initial DOCUMENT embeddings (for retrieval candidates)
        self.doc_embeddings = self._compute_initial_doc_embeddings(
    ...
    def _compute_initial_doc_embeddings(
        self, graph_data: Data | HeteroData, cutoff_year: int
    ) -> torch.Tensor:
        """Compute document embeddings using the GNN encoder."""
        ...
        if self.is_hetero:
            ...
            with torch.no_grad():
                # Use encode_documents for document embeddings
                out = self.gnn_model.encode_documents(modified_data)
            ...
            with torch.no_grad():
                # Use encode_documents for document embeddings
                return self.gnn_model.encode_documents(
                    graph_data.x,
                    edge_index_cutoff,
    ...
    def run(self, k_values: list[int] = [5, 10, 100]) -> float:
        ...
        for date, group in tqdm(grouped_by_date, desc="Evaluating"):
            ...
            if self.is_hetero:
                ...
                x_query = self.graph_data["paragraph"].x_query
            else:
                ...
                x_query = (
                    self.graph_data.x_query
                    if hasattr(self.graph_data, "x_query")
                    else self.graph_data.x
                )
            ...
            cand_emb = self.doc_embeddings[cand_indices]
            ...
            # For dual encoder: query embeddings come from MLP only (no graph needed)
            # Just get the features for query nodes and encode them
            with torch.no_grad():
                query_features = x_query[nodes_with_out_edges]
                query_emb = self.gnn_model.encode_query(query_features)
            ...
            query_ids = [
                decode_celex(node_id) for node_id in node_id_hash[nodes_with_out_edges]
            ]
            ...
            # Update document embeddings for newly seen documents
            # Need to compute GNN embeddings for these nodes now that they're "published"
            with torch.no_grad():
                if self.is_hetero:
                ...
                expanded_sub = next(iter(expanded_loader))

                if self.is_hetero:
                    doc_embeddings = self.gnn_model.encode_documents(expanded_sub)
                    if isinstance(doc_embeddings, dict):
                        doc_embeddings = doc_embeddings["paragraph"]
                    original_node_count = num_nodes
                    embeddings_to_update = doc_embeddings[:original_node_count]
                    self.doc_embeddings[nodes_with_out_edges] = embeddings_to_update
                else:
                    doc_embeddings = self.gnn_model.encode_documents(
                        expanded_sub.x,
