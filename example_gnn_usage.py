...
def train_example() -> None:
    ...
    model = GAT(
        ...
        hidden_channels=256,  # Moderate size for memory efficiency
        out_channels=512,  # Good output dimension
        num_layers=3,  # 3 layers is optimal
        dropout=0.2,  # GAT regularization
        v2=True,  # Use GATv2
        heads=2,  # 4 heads (8 was too memory-intensive)
        concat=True,  # Concat heads in intermediate layers
    )

    trainer = GNNTrainer(
        text_encoder_name=encoding_model,
        output_path="checkpoints/gnn",
        batch_size=32,  # ⭐ Much lower to fit in memory
        epochs=50,  # More epochs since smaller batches
        eval_every_n_epochs=5,
        learning_rate=3e-4,  # Lower LR for stability with small batches
        weight_decay=1e-4,
        temperature=0.05,
        num_negatives=12,
